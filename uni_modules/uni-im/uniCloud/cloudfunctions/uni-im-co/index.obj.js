...
module.exports = {
	...
	async sendMsg(params, context) {
		...
		//发送者身份id
		const from_uid = this.current_uid || original_from_uid;
		...
		// 请求公共模块中的ChatGPT_uid常量  
		// const isChatGPT = uniImConfig.config('ChatGPT_uid')
		const isPsychologist = uniImConfig.config('Psychologist_id')
		const isSurgeon = uniImConfig.config('Surgeon_id')
		const isInternist = uniImConfig.config('Internist_id')
		const isPediatrician = uniImConfig.config('Pediatrician_id')
		const isObstetrician_Gynecologist = uniImConfig.config('Obstetrician_Gynecologist_id')
		const isOphthalmologist = uniImConfig.config('Ophthalmologist_id')
		const isOtolaryngologist = uniImConfig.config('Otolaryngologist_id')
		const isOrthopedic_Surgeon = uniImConfig.config('Orthopedic_Surgeon_id')
		if (isPsychologist === to_uid) {
			try {
				// 请求chatGPT云函数  
				const {
					result: {
				...
				// 拼接消息  
				const allChatData = openAIResp.reduce((prev, {
					text
				}) => prev + text, '')
		...
		if (isObstetrician_Gynecologist === to_uid) {
			try {
				// 请求chatGPT云函数  
				const {
					result: {
		...
		if (isOrthopedic_Surgeon === to_uid) {
			try {
				// 请求chatGPT云函数  
				const {
					result: {
